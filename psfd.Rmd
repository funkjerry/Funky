...
d3 %>%
  ...
  labs(
    ...
    x="主樣本出生年度",y="平均分數")->graph$happylife
###d6 
d6 %>%
  ggplot(aes(x=scale, y=count, group=category, color=category))+
  geom_line(linetype="solid", size=2)+
...
d11 %>%
  ...
  geom_point(size=3)+
  scale_x_discrete(labels=labelsx)+
  scale_y_continuous(limits=c(1.8,3.9),
  breaks = seq(1.8,3.9, by=0.2))+
  scale_color_discrete(
  labels=c("Work","Family","Eco","Marry"))+
  theme(text=element_text(size=10, family="Comic Sans MS"),
  axis.text.x=element_text(angle=45,vjust=0.5))+
  labs(
    title="主樣本的工作、家庭、經濟、婚姻滿意度分數",
    x="主樣本出生年度",y="平均分數")->graph$satisfaction
